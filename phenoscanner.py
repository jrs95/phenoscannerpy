...
parser.add_option(
    "--catalogue",
    default="GWAS",
    help="catalogue of associations to be queried (options: None, GWAS, eQTL, pQTL, mQTL, methQTL)",
)
parser.add_option("--pvalue", default=1e-5, help="pvalue threshold")
parser.add_option(
    "--proxies",
    default="None",
    help="proxies to be queried (options: None, AFR, AMR, EAS, EUR, SAS)",
)
parser.add_option("--r2", default=0.8, help="r2 threshold")
parser.add_option("--build", default=37, help="genome build (options: 37, 38)")
parser.add_option(
    "--infile", default=None, dest="infile", help="input file prefix", metavar="FILE"
)
...
if (
    ...
    or catalogue == "pQTL"
    or catalogue == "mQTL"
    or catalogue == "methQTL"
) == False:
    sys.exit(
        "Error: catalogue has to be one of None, GWAS, eQTL, pQTL, mQTL or methQTL"
    )
...
if not (float(pvalue) > 0 and float(pvalue) <= 1):
    sys.exit(
        "Error: the p-value threshold has to be greater than 0 and less than or equal to 1"
    )

if not (float(r2) >= 0.5 and float(r2) <= 1):
    sys.exit(
        "Error: the r2 threshold has to be greater than or equal to 0.5 and less than or equal to 1"
    )
        ...
        if len(qsnps) > 100:
            ...
            for i in range(n_queries):
                query = urllib.request.urlopen(
                    "http://www.phenoscanner.medschl.cam.ac.uk/api/?snpquery="
                    + "+".join(qsnps[i * 10 : (i + 1) * 10])
                    + "&catalogue="
                    + catalogue
                    + "&p="
                    + str(pvalue)
...
if querygene == 1:
    if gene == True:
        ...
        if len(qgenes) > 10:
            ...
            for i in range(n_queries):
                query = urllib.request.urlopen(
                    "http://www.phenoscanner.medschl.cam.ac.uk/api/?genequery="
                    + qgenes[i]
                    + "&catalogue="
                    + catalogue
                    + "&p="
                    + str(pvalue)
                    + "&proxies=None&r2=1&build="
                    + str(build)
                )
                out = json.loads(query.read().decode("utf-8"))
                if "error" in out:
...
if queryregion == 1:
    if region == True:
        ...
        if len(qregions) > 10:
            sys.exit(
                "Error: a maximum of 10 region queries can be requested at one time"
            )
        else:
            ...
            if any(i > 1000000 for i in diff) or any(i < 0 for i in diff):
                ...
                for i in range(n_queries):
                    query = urllib.request.urlopen(
                        "http://www.phenoscanner.medschl.cam.ac.uk/api/?regionquery="
                        + qregions[i]
                        + "&catalogue="
                        + catalogue
                        + "&p="
